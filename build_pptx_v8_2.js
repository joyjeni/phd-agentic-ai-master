...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const rows = [
    ['Paper', 'Venue', 'Paper Title | Key Finding', 'Relevance to Obj 1'],
    ['LazyLLM (Fu et al., 2024)', 'arXiv:2407.14057', 'LazyLLM: Dynamic Token Pruning Without Training for Accelerated Inference | Dynamic token pruning; 2.34× prefill speedup on LLaMA-2-7B', 'SessionRerank+ dynamic pruning component'],
    ['H2O (Zhang et al., 2023)', 'NeurIPS 2023', 'H2O: Heavy-Hitter Oracle for Efficient Generative Inference of LLMs | "Heavy-hitter" tokens; eviction cuts KV memory 20×', 'Activity-score-driven KV eviction in SessionRerank+'],
    ['Scissorhands (Liu et al., 2023)', 'NeurIPS 2023', 'ScissorHands: Exploiting the Persistence of Importance Hypothesis for LLM KV Cache Compression | Pivot-set KV cache; 5× memory reduction', 'Temporal stability basis for activity-history weighting'],
    ['StreamingLLM (Xiao et al., 2024)', 'ICLR 2024', 'Efficient Streaming LLMs with Attention Sinks | Attention sinks enable infinite streaming; 22.2× faster', 'Session-init tokens as permanent anchors in reranking'],
    ['GEAR (Kang et al., 2024)', 'arXiv:2403.05527', 'GEAR: An Efficient KV Cache Compression Recipe via Low-Rank Perturbation | Low-rank + sparse KV compression; <0.5% accuracy drop at 4-bit', 'Low-rank decomposition for session-history states'],
    ['CacheGen (Liu et al., 2024)', 'SIGCOMM 2024', 'CacheGen: KV Cache Compression and Streaming for Fast LLM Serving | Compressed KV bitstream streaming; 3.7× bandwidth reduction', 'Persistent-session KV serialisation across inference steps'],
    ['MiniCache (Liu et al., 2024)', 'NeurIPS 2024', 'MiniCache: KV Cache Compression in Depth Dimension for LLMs | Cross-layer token merging; depth-dimension KV compression', 'Depth-aware KV fusion in SessionRerank+'],
    ['ToolLLM (Qin et al., 2024)', 'ICLR 2024', 'ToolLLM: Facilitating LLMs to Master 16000+ Real-World APIs | DFSDT on 16k+ APIs; ToolBench evaluation framework', 'Benchmark and tool-routing baseline for SessionRerank+'],
  ];

  const colW = [2.6, 1.8, 5.3, 2.95];
  const colX = [0.3, 2.95, 4.8, 10.15];
  const rowH = 0.54;
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const rows = [
    ['Paper', 'Venue', 'Paper Title | Key Finding', 'Relevance to Obj 2'],
    ['RouteLLM (Ong et al., 2024)', 'arXiv:2406.18665', 'RouteLLM: Learning to Route LLMs with Preference Data | Offline preference training; reduces GPT-4 calls 50%', 'Demonstrates routing viability; Adaptive Priority-aware Request Router (APRR)+CDR+PDR addresses offline-only limitation'],
    ['FrugalGPT (Chen et al., 2023)', 'arXiv:2305.05176', 'FrugalGPT: How to Use LLMs While Reducing Cost and Improving Performance | Cascade routing; fixed policy offline', 'Baseline cost-routing; lacks online Reinforcement Learning (RL) adaptation (addressed by APRR)'],
    ['Mixture-of-Agents (Wang et al., 2024)', 'arXiv/COLM', 'Mixture-of-Agents Enhances LLM Capabilities | Layered agent aggregation; no dynamic routing', 'Motivates parallel dispatch — Parallel Dispatch Routing (PDR) extends this with functional-token routing'],
    ['TensorOpera Router (Stripelis, 2024)', 'EMNLP 2024', 'TensorOpera Router: A Multi-Model Router for Efficient LLM Inference | Hardware-aware scheduling', 'Infrastructure context for Context-Driven Routing (CDR) deliberation scoring'],
    ['RouterDC (Chen et al., 2024)', 'arXiv', 'RouterDC: Query-Based LLM Router Training with Dual Contrastive Learning | Dual contrastive routing', 'Contrastive objective comparable to CDR quality scoring'],
    ['PickLLM (Sikeridis et al., 2024)', 'arXiv', 'PickLLM: Context-Aware RL-Assisted LLM Routing | Context-aware RL routing', 'Closest prior art to APRR; lacks exponential decay + binary session signals'],
    ['GraphRouter (Feng et al., 2024)', 'arXiv', 'GraphRouter: A Graph-Based Router for LLM Selections | Graph routing with relational reasoning', 'Graph routing vs. CDR chain-of-thought deliberation comparison'],
    ['Eagle (Zhao et al., 2024)', 'arXiv', 'Eagle: Efficient Training-Free Router for Multi-LLM Inference | Training-free router via capability matching', 'Training-free baseline; APRR+CDR+PDR surpasses with online RL'],
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const rows = [
    ['Paper', 'Venue', 'Paper Title | Key Finding', 'Relevance to Obj 3'],
    ['AgentNet (Yang et al., 2025)', 'arXiv cs.MA', 'AgentNet: Decentralised Evolutionary Coordination for LLM Agents | Emergent specialisation via decentralised coordination', 'Decentralisation model for MNCD node topology'],
    ['MAS Reliability (Zheng et al., 2025)', 'arXiv cs.MA', 'Towards Reliable Multi-Agent Systems: A Byzantine Fault-Tolerant Perspective | Byzantine fault model for MAS reliability', 'Fault model validation for MNCD 2-of-5 failure scenario'],
    ['MAS-FIRE (Jia et al., 2026)', 'arXiv cs.SE', 'MAS-FIRE: Fault Injection and Reliability Evaluation for LLM Multi-Agent Systems | Fault injection benchmarking for LLM MAS', 'Evaluation framework for MNCD fault-tolerance benchmarking'],
    ['MorphAgent (Lu et al., 2024)', 'arXiv cs.AI', 'MorphAgent: Empowering LLM Agents Through Self-Evolving Profiles | Decentralised memory with self-evolving profiles', 'Decentralised profile store analogous to MNCD context nodes'],
    ['LLM Multi-Agent Survey (Han et al., 2024)', 'arXiv:2402.01680', 'A Survey on LLM-based Autonomous Agents (Han et al.) | Open problems: fault tolerance, context sharing, coordination', 'Positions MNCD within the open-problem landscape'],
    ['Gossip Protocol in Fed. Learning (Husnoo, 2024)', 'arXiv cs.CR', 'Gossip Protocol-Based Federated Learning for Anomaly Detection | P2P gossip for distributed AI context propagation', 'Gossip-protocol broadcast validated for distributed AI context'],
    ['Gossip P2P Learning (Naik et al., 2023)', 'IEEE ICTBIG', 'An Introduction to Gossip Protocol Based Learning in P2P Federated Learning (Naik et al.) | O(log n) propagation rounds', 'MNCD propagation rounds baseline (O(log n))'],
    ['FedCod (Yan et al., 2024)', 'arXiv cs.DC', 'FedCod: An Efficient Communication Protocol for Cross-Silo Federated Learning with Coding | Coded protocol; 62% communication time reduction', 'Communication efficiency bounds for MNCD propagation'],
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const rows = [
    ['Paper', 'Venue', 'Paper Title | Key Finding', 'Relevance to Obj 4'],
    ['Tero et al. (2010)', 'Science', 'Rules for Biologically Inspired Adaptive Network Design | Adaptive transport network formalism; flow-controlled pruning rules', 'Flow-controlled pruning formalism adapted to token-graph topology'],
    ['Tero et al. (2007)', 'J. Theor. Biol.', 'A Mathematical Model for Adaptive Transport Network in Path Finding | Adaptive network path-finding model', 'Path reinforcement model basis for FCNP flow-score pruning'],
    ['Bonifaci et al. (2012)', 'J. Theor. Biol.', 'Physarum Can Compute Shortest Paths: Convergence Proof | Shortest path convergence for adaptive network dynamics', 'Convergence guarantee for FCNP iterative pruning'],
    ['LoRA (Hu et al., 2022)', 'ICLR 2022 — arXiv:2106.09685', 'LoRA: Low-Rank Adaptation of Large Language Models | Low-rank decomposition for parameter-efficient fine-tuning', 'Low-rank adaptation principle for FCNP weight updates'],
    ['SparseGPT (Frantar et al., 2023)', 'ICML 2023', 'SparseGPT: Massive Language Models Can Be Accurately Pruned in One-Shot | One-shot pruning with minimal accuracy loss', 'One-shot pruning baseline compared against FCNP iterative compression'],
    ['Scissorhands (Liu et al., 2023)', 'NeurIPS 2023 — arXiv:2305.17118', 'ScissorHands: Exploiting the Persistence of Importance Hypothesis for LLM KV Cache | Importance persistence for selective KV retention', 'Token importance stability reused in FCNP context-graph scoring'],
    ['AdaKV (Ge et al., 2024)', 'ICLR 2024 — arXiv:2407.11550', 'AdaKV: Optimising KV Cache Eviction by Adaptive Budget Allocation | Adaptive KV budget guided by model internals; XAI-aligned interpretable scoring', 'Dynamic budget allocation strategy for FCNP per-agent allocation'],
    ['Finch (Corallo et al., 2024)', 'TACL 2024 — arXiv:2408.00167', 'Finch: Prompt-guided Key-Value Cache Compression for LLMs | Prompt-guided KV compression; minimal citation loss', 'Prompt-guided scoring complements FCNP citation-preserving compression'],
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const implItems = [
    ...
    { label: 'Free Compute (HW)', value: 'Kaggle Notebooks: NVIDIA Tesla T4 ×2 (16 GB VRAM each, free tier) | Google Colab T4 (free) / A100 (Pro+)\nNo paid GPU used — all experiments reproducible on free academic compute', color: C.teal },
    { label: 'Libraries (Free SW)', value: 'PyTorch 2.1 | HuggingFace Transformers 4.40 | FAISS-cpu 1.7 | IndicTrans2 (AI4Bharat, Apache 2.0) | Gradio 4.x | sentence-transformers 2.x | scikit-learn | pandas | numpy\nPlanned: vLLM (inference acceleration), PEFT/LoRA, Weights & Biases (experiment tracking)', color: C.indigo },
    ...
    { label: 'Pilot Metric (In Progress)', value: 'NDCG@5 = 0.461 | n = 500 test queries | p < 0.0001 (Wilcoxon signed-rank vs dense retrieval) | Full study target: NDCG@5 ≥ 0.52', color: C.green },
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const implItems = [
    ...
    { label: 'Free Compute (HW)', value: 'Kaggle Notebooks: NVIDIA Tesla T4 ×2 (16 GB VRAM each, free tier) | Google Colab T4 (free) / A100 (Pro+)\nAll routing experiments run on free Kaggle/Colab GPU — no paid cloud required', color: C.teal },
    { label: 'Libraries (Free SW)', value: 'PyTorch 2.1 | HuggingFace Transformers 4.40 | NumPy | scikit-learn | ToolBench executor (open-source) | Vercel (free hobby tier)\nPlanned: Ray (distributed RL), Optuna (hyperparameter tuning), MLflow (experiment tracking), FastAPI', color: C.green },
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const implItems = [
    ...
    { label: 'Free Compute (HW)', value: 'Kaggle Notebooks: NVIDIA Tesla T4 ×2 (16 GB VRAM each, free tier) | Google Colab T4 (free)\n5-node mesh simulated via asyncio coroutines on single GPU — no paid cloud required', color: C.teal },
    { label: 'Libraries (Free SW)', value: 'Python 3.10 | asyncio | gRPC (Apache 2.0) | HuggingFace Transformers 4.40 | google/gemma-2-2b-it or Qwen/Qwen2.5-7B-Instruct (open weights)\nPlanned: Apache Kafka (event streaming), Redis (context TTL store), Prometheus + Grafana (mesh observability)', color: C.green },
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  slide.addText('Flow-Conductance-based Neural Pruning (FCNP) compresses multi-agent conversation context graphs through iterative flow-score-based token pruning. FCNP incorporates Explainable AI (XAI) transparency: pruning decisions are fully logged via flow-score traces, enabling auditable token eviction.', {
    x: 0.35, y: 0.58, w: 12.6, h: 0.38,
    fontSize: 13, italic: true, color: C.deepPurple, fontFace: 'Calibri', wrap: true,
  });
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const implItems = [
    ...
    { label: 'Free Compute (HW)', value: 'Kaggle Notebooks: NVIDIA Tesla P100 (16 GB VRAM, free tier) + T4 ×2 | Google Colab A100 (Pro+)\nFlashAttention-2 requires Ampere GPU (A100) for full speed — Colab Pro+ used for final benchmarks', color: C.teal },
    { label: 'Libraries (Free SW)', value: 'PyTorch 2.1 | FlashAttention-2 (Tri Dao, Apache 2.0) | HuggingFace Transformers 4.40 | PEFT/LoRA | bitsandbytes (4-bit quantisation) | Vercel (free tier)\nPlanned: NetworkX (graph construction), SciPy (Kirchhoff solver), Weights & Biases, vLLM', color: C.green },
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const NOV_OBJ = [
    { label: 'Obj 1\nSessionRerank+',
      ...
      gain: 'NDCG@5: 0.452 → 0.461 (+2%, p<0.05); Precision@5: +1.8%; MRR: +3.1%; Domain accuracy (data.gov.in): 78.4%', gainTxt: '14532E', gainBg: 'DCFCE7' },
    { label: 'Obj 2\nAPRR+CDR+PDR',
  ...
  const feasCols = [
    {
      ...
      items: [
        ...
        'Risk: data.gov.in API downtime \u2192 Mitigation: graceful degradation — system falls back to 24h cached mandi feed; SessionRerank+ continues on cached context (no crash)',
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const phases = [
    ...
    },
    {
      ...
      tasks: [
        'Phase 1 Foundational Validation: Ablation studies on SessionRerank+ and FCNP vs. baselines on TREC Session (nDCG@5, Precision@k, MRR)',
        'Phase 2 Domain Adaptation: Fine-tune ACRS on data.gov.in AgriGov corpus; domain-specific accuracy on agricultural queries',
        'Phase 3 Real-World Simulation: Deploy prototype on cloud (AWS/GCP); stress tests, API downtime resilience, graceful degradation metrics',
